...
var handler *handlers.DeviceUnsubscribeHandler
...
func init() {
	...
	if err := c.Invoke(func(h *handlers.DeviceUnsubscribeHandler) {
		handler = h
	}); err != nil {
		panic(err)
	}
...
// main starts the Lambda api gateway that unsubscribes user devices to a specific topic
func main() {
